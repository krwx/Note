...
- 但是可以通过使用 v-bind 来将v-model 绑定的值该值绑定到当前组件实例上的动态数据。
...
# 生命周期
参考生命周期钩子 API 。
![生命周期图](https://cn.vuejs.org/assets/lifecycle.16e4c08e.png)
...
# 模板引用
直接访问底层 DOM 元素可以使用特殊的 **ref** 。允许我们在一个特定的 DOM 元素或子组件实例被挂载后，获得对它的直接引用attribute。
