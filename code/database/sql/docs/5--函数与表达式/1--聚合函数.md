...
2.1、返回 Products 表中 Price 列的最低价格：
...
2.2、返回 Employees 表中 BirthDate 列的最早的出生日期：
...
2.3、与 ```GROUP BY``` 一起使用。返回 Products 表中每个类别的最低价格：
...
2.2、计算 ProductName 列中所有非 NULL 值的数量
...
2.2、列出所有价格高于平均值的记录，在子查询中使用 `AVG()` 函数：
