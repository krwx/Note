...
语法：`class pandas.ExcelFile(path_or_buffer, engine=None, storage_options=None, engine_kwargs=None)`
...
解析一个或多个表单并返回一个 DataFrame

语法：`ExcelFile.parse(sheet_name=0, header=0, names=None, index_col=None, usecols=None, converters=None, true_values=None, false_values=None, skiprows=None, nrows=None, na_values=None, parse_dates=False, date_parser=<no_default>, date_format=None, thousands=None, comment=None, skipfooter=0, dtype_backend=<no_default>, **kwds)`

## ExcelWriter

ExcelWriter 是 pandas 提供的一个类，用于将 DataFrame 或 Series 对象写入 Excel 文件。使用 ExcelWriter，你可以在一个 Excel 文件中写入多个工作表，并且可以更灵活地控制写入过程。

语法：`class pandas.ExcelWriter(path, engine=None, date_format=None, datetime_format=None, mode='w', storage_options=None, if_sheet_exists=None, engine_kwargs=None)`
...
你可以使用同一个 ExcelWriter 对象将不同的 DataFrame 写入同一个 Excel 文件的**不同工作表**。
...
向**同一个工作表**写入**多个 DataFrame**，注意 `if_sheet_exists` 参数需要设置为 `overlay`：
...
将 DataFrame 写进 excel 文件，并将 excel 文件打包到 zip 压缩文件中：
