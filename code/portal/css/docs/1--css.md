...
|:is()|:is() CSS 伪类函数以选择器列表作为参数，并选择该列表中任意一个选择器可以选择的元素。但是不能选择伪元素。<br>例如 `:is(ol, ul)`，它代表 `ol ol`、`ol ul`、`ul ol`、`ul ul`这四个选择器。|
...
有**两条相同权重的规则，后面的规则会应用**。可以理解为后面的规则覆盖前面的规则，直到最后一个开始设置样式。  
