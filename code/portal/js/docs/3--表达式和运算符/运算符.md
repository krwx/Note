...
原因：在 JavaScript 最初的实现中，JavaScript 中的值是由一个表示类型的标签和实际数据值表示的。对象的类型标签是 0。然后由于 `null` 代表的是空指针（大多数平台下值为 0x00），`null` 的类型标签是 0，所以 `typeof null` 返回 `"object"`。

#### 使用 new 操作符

所有使用 `new` 调用的构造函数都将返回 `"object"` 或 `"function"`。但是 `typeof Function` ，它返回 `"function"`。
...
但是 `let` 和 `const` 声明的变量，如果在声明前使用 `typeof` 则会抛出一个 `ReferenceError`
...
思路：调用 Object 原型的 `toString()` 获取 `"[object Type]"` ，然后截取字符串，取 `"object"` 后面的部分
