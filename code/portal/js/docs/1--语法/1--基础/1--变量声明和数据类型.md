...
1. **变量声明提升**：
   - 使用 `var` 声明的变量会被提升到其作用域的顶部，但只提升声明，不提升赋值。初始值为 `undefined`。可以在声明前访问（得到 undefined）。
   - 使用 `let` 和 `const` 声明的变量也存在提升，**但不会初始化**。从作用域开始到声明之间的区域称为 **暂时性死区（TDZ, Temporal Dead Zone）**，在此期间访问变量会抛出 `ReferenceError`。
2. **函数声明提升**：
