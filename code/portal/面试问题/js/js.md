...
- [js](#js)
  - [1. 函数柯里化](#1-函数柯里化)
  - [2. let 和 const 为什么不像 var 能在变量声明赋值前被使用 / 说明变量提升的理解](#2-let-和-const-为什么不像-var-能在变量声明赋值前被使用--说明变量提升的理解)
  - [3. 解释 let 和 const 为什么不像 var 能在变量声明赋值前被使用](#3-解释-let-和-const-为什么不像-var-能在变量声明赋值前被使用)
  - [4. 简单介绍原型链 / 讲一下原型是什么](#4-简单介绍原型链--讲一下原型是什么)
  ...
  - [56. instanceof 是干什么的，实现原理是什么？](#56-instanceof-是干什么的实现原理是什么)
  ...
  - [68. 了解 js 的运行流程吗 / 讲一下任务队列、宏任务、微任务，它们是怎么工作的 / 宏任务有哪些、微任务有哪些 / 宏任务和微任务哪个的优先级高](#68-了解-js-的运行流程吗--讲一下任务队列宏任务微任务它们是怎么工作的--宏任务有哪些微任务有哪些--宏任务和微任务哪个的优先级高)
  - [69. 讲一下栈、堆、队列的区别](#69-讲一下栈堆队列的区别)
  - [70. 箭头函数有哪些用处](#70-箭头函数有哪些用处)
  - [71. 了解 js 的模块开发吗](#71-了解-js-的模块开发吗)
  - [72. js 是怎么导入模块的](#72-js-是怎么导入模块的)
  - [73. 我现在不用 ajax 请求，改用 web socket 请求，怎么改代码](#73-我现在不用-ajax-请求改用-web-socket-请求怎么改代码)
  - [74. 假设现在有一个入参，然后传递给后端，后端需要返回 json 格式。你怎么去做](#74-假设现在有一个入参然后传递给后端后端需要返回-json-格式你怎么去做)
  - [75. 假如让你做一个拓扑组件你怎么做](#75-假如让你做一个拓扑组件你怎么做)

## 1. 函数柯里化

柯里化：是把接受多个参数的函数变换成接受一个单一参数（最初函数的第一个参数）的函数，并且返回接受余下的参数而且返回结果的新函数的技术。

比如把 `f(a, b, c)` 转换成 `f(a)(b)(c)`。
...
主要概念就是让函数返回一个 function 可以接受第二个（或者更多）括号里的参数并输出期望值。
...
## 2. let 和 const 为什么不像 var 能在变量声明赋值前被使用 / 说明变量提升的理解

var，let 和 const 都存在变量提升，但是 var 声明的变量会被初始化为 undefined，而 let 和 const 声明的变量不会被初始化，在声明之前访问会抛出 ReferenceError，这个阶段叫做暂时性死区（Temporal Dead Zone，TDZ）。因此，let 和 const 声明的变量必须在声明后才能使用，而 var 声明的变量可以在声明前访问（但值为 undefined）。

## 3. 解释 let 和 const 为什么不像 var 能在变量声明赋值前被使用

造成变量声明提升的底层机制：

**1. 执行上下文与环境记录**

每个作用域对应一个 执行上下文（Execution Context），其中包含：

- **变量环境（VariableEnvironment）**：处理 `var` 声明的变量。
- **词法环境（LexicalEnvironment）**：处理 `let`、`const` 以及 `function` 声明的绑定。

**2. 编译阶段（声明提升的实质）**

引擎在进入作用域时，会先进行声明性环境记录的实例化：

- 对于 `let` 和 `const`：引擎会为每个变量在词法环境中创建一个绑定（binding），但该绑定的初始化状态被标记为 `uninitialized`。
- 对于 `var`：在变量环境中创建绑定，并立即初始化为 `undefined`。

这个“创建绑定”的动作就是“提升”的本质——变量名在作用域开始就已经被引擎知道。

**3. 暂时性死区（TDZ）的底层实现**

引擎在执行代码时，访问变量会检查其绑定状态：

- 如果当前绑定状态是 `uninitialized`，则抛出 `ReferenceError`。
- 只有当执行流运行到 `let` 或 `const` 声明语句时，引擎才会将该绑定的状态从 `uninitialized` 改为 `initialized`，并同时赋值（对 `const` 必须赋值）。

因此，从进入作用域到声明语句之间的区域，绑定的状态始终为 `uninitialized`，这就是 TDZ。
...
为 null，`Object.prototype.__proto__ == null`
...
async/await 是 ES2017 特性，现代移动端浏览器和 WebView 已广泛支持，但部分老旧环境（如 Android 5.0 以下系统 WebView、iOS 9 及更早的 Safari）不支持。

可以使用 Babel 转译 + Polyfill 将 async/await 转换为 ES5 代码，并注入运行时依赖。
...
放弃回答：我目前参与的项目主要面向现代浏览器（Chrome/Firefox/Safari最新版），所以直接的兼容性问题不多。
...
| 特性 | eventBus | Redux |
| ------ | ---------- | ------- |
| **模式** | 发布-订阅（Pub/Sub） | 单向数据流 + 单一状态树 |
| **存储** | 没有统一状态，仅传递事件 | 全局单一 Store 存储所有状态 |
| **数据流** | 任意组件均可触发/监听事件，方向自由 | 严格单向：`View → Action → Reducer → Store → View` |
| **状态可预测性** | 低（事件顺序、来源难以追踪） | 高（每次状态变更都通过 Action 触发） |

## 17. 在什么时候使用 eventBus

- 极简单的跨组件通信（如兄弟组件传值）。
- 非状态驱动的全局通知（如 toast 弹窗、播放器控制）。
- 小型项目或原型开发。
...
1. 不正当的闭包  
    `fn2Child` 它是闭包，并且因为 return 的函数中存在函数 `fn2` 中的 `test` 变量引用，所以 `test` 并不会被回收，也就造成了内存泄漏。
...
2. 意外的全局变量
   - 由于使用未声明的变量，而意外的创建了一个全局变量，在函数执行完毕后，无法回收该变量。
   - 使用 this 也可以创建全局变量，例如：
...
4. 脱离 DOM 的引用
   - 获取一个 DOM 元素的引用，而后面这个元素被删除，由于我们一直保留了对这个元素的引用，所以它也无法被回收。需要解除引用
   - 还要考虑 DOM 树内部或子节点的引用问题。例如：代码中保存了表格某一个 `<td>` 的引用，当回收时，`<td>` 所在的表格不会参与回收
...
闭包（closure）是由函数以及函数所在的词法环境组合而成的。
...
[内存泄漏的介绍](../../js/docs/1--语法/2--中级/1--闭包.md#1-内存泄漏)
...
**分析思路**：分析从 `GC Root` 能不能访问到该值，访问到则不回收，访问不到则回收

**情况一：闭包捕获了 A 实例的引用（最常见）**

```javascript
class A {
  constructor() {
    this.data = new Array(1000).fill('*'); // 大内存
  }
  getClosure() {
    // 这个箭头函数捕获了 this（即 A 实例）
    return () => console.log(this.data.length);
  }
}

let closure = new A().getClosure(); // A 实例没有变量直接引用，但闭包持有它
// 此时 closure 被某个 B 对象长期持有
```

- `new A().getClosure()` 返回的闭包持有 **A 实例**（因为 `this` 被捕获）。
- 即使没有变量直接引用 `A` 实例，但通过 `closure` 这条路径，A 实例仍然**是可达的**。
- 结果：**垃圾回收一定不成功**。只要 B 还在用这个闭包，A 实例就永远不会被回收。

**情况二：闭包没有捕获 A 实例，只捕获了局部变量**

```javascript
class A {
  createClosure() {
    let localBigData = new Array(1000).fill('*');
    return () => console.log(localBigData.length);
    // 没有捕获 this，只捕获 localBigData
  }
}

let closure = new A().createClosure(); 
// A 实例本身没有被闭包引用
```

- 这里闭包只捕获了 `localBigData`（它是函数执行时创建的对象）。
- A 实例创建后，如果不再有其他引用，A 实例**可以被回收**（尽管它的方法创建了闭包）。
- 但注意：`localBigData` 不会被回收，因为它被闭包持有，而闭包被 B 长期使用。
...
2. 计算中间位置的索引。
3. 比较中间位置的值与目标值。
4. 根据比较结果调整查找范围。如果中间值等于目标值，返回索引；如果中间值大于目标值，缩小右边界；如果中间值小于目标值，缩小左边界。
5. 重复步骤 2-4，直到找到目标值或查找范围为空。
...
[Promise](../../js/docs/2--内置对象/Promise.md#手写-promise)

## 47. 创建对象的方法，除了Object.create 和 function

[创建对象](../../js/docs/1--语法/1--基础/11--对象.md#创建对象的方式)
...
- 箭头函数表达式没有自己的 `this`，`arguments`，`super` 或 `new.target`。不能用作构造函数。
- 箭头函数不绑定 `this`，只会从自己的作用域链的上一层继承 `this`
- 箭头函数没有 `prototype` 属性。
- `yield` 关键字通常不能在箭头函数中使用（除非是嵌套在允许使用的函数内）。因此，箭头函数不能用作函数生成器。
- 由于箭头函数没有自己的 `this` 指针，通过 `call()` 或 `apply()` 方法调用一个函数时，**只能传递参数（不能绑定 `this`），他们的第一个参数 `this` 会被忽略**。

## 56. instanceof 是干什么的，实现原理是什么？
...
- 变量提升：
  ...
  - `let` 和 `const` 也支持变量提升，但是在声明语句前调用会报 `ReferenceError` 错误。
- 重复声明：
...
promise 的异常穿透是进行链式调用的时候才会出现异常穿透；
...
## 68. 了解 js 的运行流程吗 / 讲一下任务队列、宏任务、微任务，它们是怎么工作的 / 宏任务有哪些、微任务有哪些 / 宏任务和微任务哪个的优先级高

**1. 核心机制**

“JavaScript 是**单线程**的语言，它通过**事件循环**机制来处理异步操作，从而实现非阻塞的 I/O。”

**2. 运行流程**

“整个运行流程可以分为几个核心部分：**调用栈**、**Web APIs（或宿主环境）**、**任务队列（包括宏任务队列和微任务队列）**，以及不断工作的**事件循环**。”

**3. 流程仔细说明**

1. **执行同步代码**：
    - 首先，JS 引擎会从上到下执行全局的同步代码，将函数调用压入**调用栈**，执行完毕后弹出。
    - 遇到 `console.log`、变量赋值等同步操作，立即执行。

2. **处理异步任务**：
    - 当遇到异步操作（如 `setTimeout`、`fetch`、`Promise`、`addEventListener`、`fs.readFile` 等）时，JS 引擎不会阻塞等待，而是将它们交给**宿主环境（浏览器或 Node.js）** 去处理。
    - 同时，会继续执行调用栈中剩下的同步代码。

3. **任务注册与回调**：
    - 宿主环境处理完异步任务后（例如定时器到点、请求返回数据），会将回调函数放入对应的队列：
        - **宏任务队列**：`setTimeout`、`setInterval`、I/O 操作、UI 渲染、`setImmediate`（Node.js）等。
        - **微任务队列**：`Promise.then/catch/finally`、`MutationObserver`、`queueMicrotask`、`process.nextTick`（Node.js）等。
    - 注意：**微任务的优先级高于宏任务**。

4. **事件循环不断工作**：
    - **事件循环**会持续检查：
        - 首先，检查**调用栈**是否为空。
        - 如果为空，则先清空**所有微任务**（执行微任务队列中的所有回调，直到队列为空）。
        - 然后，从**宏任务队列**中取出**一个**宏任务，压入调用栈执行。
        - 执行这个宏任务的过程中，如果又产生了新的微任务，会继续在本轮清空微任务队列。
        - 如此循环往复。

## 69. 讲一下栈、堆、队列的区别

- **栈（调用栈 Call Stack）**  
  - 存储：基本类型（number, string, boolean, undefined, null, symbol, bigint）以及函数执行上下文。  
  - 特点：固定大小，由 JS 引擎自动管理，函数调用时会创建栈帧，执行完就销毁。  
  - 风险：递归过深会栈溢出（`Maximum call stack size exceeded`）。

- **堆（Heap）**  
  - 存储：引用类型（对象、数组、函数等）。  
  - 特点：无结构、无序内存池，大小不固定，需要垃圾回收（GC）来清理无用对象。  
  - 注意：栈中只保存堆对象的引用地址，真正的数据在堆里。

- **队列（任务队列 Task Queue / 事件队列）**  
  - 存储：异步任务的回调（setTimeout、Promise.then、事件回调等）。  
  - 工作机制：配合**事件循环**，当调用栈清空后，从队列中取出回调放入栈执行。  
  - 分类：微任务队列（Promise、MutationObserver）优先级高于宏任务队列（setTimeout、I/O）。

> - “栈和堆哪个快？” → 栈快（连续内存、无需查找指针）。
> - “队列是内存结构吗？” → 逻辑上是队列，底层内存可以是数组或链表，但 JS 引擎实现时会优化。
> - “如何模拟栈溢出？” → 写一个无限递归函数。

## 70. 箭头函数有哪些用处

> “箭头函数主要有三个用处：  
> 第一，**语法更简洁**，适合写 map、filter、Promise.then 这类简短的函数表达式，提高可读性。  
> 第二，也是最重要的，**它不绑定自己的 this**，而是继承外层函数作用域的 this。这解决了传统函数在回调、定时器、异步操作中 this 丢失的问题，不再需要写 `var that = this` 或 `.bind(this)`。  
> 第三，**不能作为构造函数**，也没有 prototype 和 arguments，这让它的意图更纯粹，适合做纯函数或工具函数。  
> 当然，它也有局限：不能用作对象方法（如果需要访问对象属性）、不能用于 DOM 事件监听（需要动态 this 时）、不能用 new。  
> 所以我的原则是：**需要保留 this 上下文或写短函数时用箭头函数；需要动态 this、构造对象或真正的方法时用普通函数**。”

## 71. 了解 js 的模块开发吗

JS 模块化就是把代码拆成职责清晰、可复用的独立单元，对外暴露接口，对内隐藏实现。它主要解决全局污染、代码耦合和维护困难的问题。

模块化经历过 CommonJS、AMD 等阶段，现在前端主流是 ES Module，用 import/export。ESM 是官方标准，支持静态分析，配合 Vite、Webpack 能做 Tree Shaking 和懒加载。

在实际项目里，我一般会按照业务或职责拆模块，比如把 API 请求、工具函数、状态管理、UI 组件分开。组件内部也会注意单一职责，避免模块过大。结合 Vite、Webpack 这类构建工具，还可以做动态导入，比如 `import()`，实现路由级别的懒加载，优化首屏性能。

## 72. js 是怎么导入模块的

> “JS 导入模块主要有两种主流方案：  
> **CommonJS**（Node.js 常用）用 `require()` 同步导入，在运行时执行；  
> **ES Modules**（浏览器和现代 Node.js 推荐）用 `import` 静态导入，支持编译时优化和 tree shaking，也支持动态 `import()` 做懒加载。  
> 在浏览器中使用 ESM 需要把 `<script>` 标签的 `type` 设为 `module`；在 Node.js 中，若要使用 ESM 可以设置 `"type": "module"` 或使用 `.mjs` 扩展名。  
> 开发中通常会配合 Webpack、Vite 等打包工具，将模块转换成浏览器兼容的代码。”

## 73. 我现在不用 ajax 请求，改用 web socket 请求，怎么改代码

按步骤来讲：

**第一步，先讲判断，不直接讲代码。**

这类问题我会先判断业务场景，因为 Ajax 和 WebSocket 不是简单的替换关系。Ajax 是 HTTP 的请求响应模型，适合普通数据获取和提交；WebSocket 是长连接双向通信，适合实时消息、推送、协同这类场景。所以我会先确认这个需求是不是一定要实时，如果只是普通接口，其实没必要强行改成 WebSocket。

**第二步，再讲整体改造思路。**

如果业务确实需要改，我会从两个层面去做。

第一是协议层，原来 ajax 是通过 URL、method、query、body 来表达语义，改成 WebSocket 之后，前后端要先定义统一的消息协议，比如 message type、requestId、payload、timestamp、code 这些字段，否则后面很容易失控。

第二是客户端层，我会把原来零散的接口调用收敛成一个 WebSocket manager，专门处理连接建立、鉴权、心跳、断线重连、消息分发和请求响应映射。因为 WebSocket 本身只有 send 和 onmessage，如果业务还想保留类似 async/await 的调用体验，就需要基于 requestId 封装一层 Promise，把返回结果和请求对应起来。

**第三步，最后补一点实现细节。**

落到代码上，前端会先创建 WebSocket 连接，发送时用 send 发消息，接收时在 onmessage 里按消息类型或者 requestId 去处理。如果业务侧还想保留类似原来 async/await 的调用方式，我会再封装一层 Promise，把一次发送和一次响应对应起来。同时还要补上心跳、断线重连、超时处理和鉴权这些生产能力。

## 74. 假设现在有一个入参，然后传递给后端，后端需要返回 json 格式。你怎么去做

我会先明确入参格式，然后在前端做参数校验，通过 `fetch` 或 `axios` 发起请求。如果后端要求返回 JSON，我会按约定设置请求头和请求体，比如 `application/json`。后端处理完后返回统一结构的 JSON，比如 `code`、`message`、`data`。前端拿到结果后根据状态做成功展示、错误提示和异常兜底。如果是实际项目，我还会关注接口文档约定、类型定义和超时重试这些问题。”

## 75. 假如让你做一个拓扑组件你怎么做

> “如果我需要从头设计一个拓扑组件，我会分四步来做：**需求澄清、技术选型、核心架构、难点处理**。
>
> **第一步，先明确边界。**  
> 我会问清楚：数据量是几百还是上万？交互需不需要拖拽、缩放、编辑？节点和边的样式是否固定？这决定了渲染方案和复杂度。
>
> **第二步，选型。**  
> 节点少于 500 且交互多，我会用 SVG，事件方便；如果超过 1000 或需要动画，我会选 Canvas。面试中我倾向于说用 Canvas，性能可控。
>
> **第三步，架构分层设计。**  
>
> 1. **数据层**：定义 Node 和 Edge 模型，支持增删改，用发布订阅触发重绘。  
> 2. **渲染层**：实现画布平移/缩放，循环绘制节点、边、文字。  
> 3. **交互层**：处理拖拽节点、画布拖拽、点击选中、悬停高亮。Canvas 下需要自己实现命中检测，我会用**四叉树**加速查找。  
> 4. **布局层**：提供力导向、树形等算法，并放到 Web Worker 里跑，避免卡顿。
>
> **第四步，攻克难点。**  
>
> - 比如 Canvas 的事件拾取：通过矩阵逆变换得到真实坐标，再用四叉树筛选候选节点，计算几何距离即可。  
> - 性能方面：用 `requestAnimationFrame` 节流重绘，拖拽时暂停布局计算，松手后再更新。  
> - 可扩展性上：设计成框架无关的类，支持插件（小地图、导出图片），并提供 React/Vue 的封装。
>
> 如果项目工期紧，我也会评估 AntV G6 这类库，但会做一层封装，方便后续替换。**总结一句话：先问清场景，再用分层架构保证性能与可维护性。**”
