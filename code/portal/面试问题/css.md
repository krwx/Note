...
- [css](#css)
  ...
  - [7. 行内布局的特点](#7-行内布局的特点)
  - [8. 说一下块级上下文/ BFC 是什么](#8-说一下块级上下文-bfc-是什么)
  ...
  - [29. 你怎么理解BFC？用过它解决过什么布局问题？](#29-你怎么理解bfc用过它解决过什么布局问题)

## 1. 移动端布局有哪些 / 响应式布局是什么 / 响应式布局有哪些，怎么使用

1. 流式布局  
    流式布局就是百分比布局。使用百分比（%）来定义宽度，使容器根据父容器或视口宽度自适应。
2. flex布局  
    通过 `display: flex` 开启，利用 `flex-grow`、`flex-shrink`、`flex-basis` 实现自适应。
3. grid 布局  
    通过 `display: grid` 开启，定义行列模板，实现复杂的响应式网格布局。
4. meta 媒体查询  
    meta 标签，通过媒体查询 按照不同手机的像素宽高不同，进行条件匹配
    ...
    搭配 rem 使用：rem适配布局
    - em：子元素字体大小单位如果使用 em 是相对于父元素字体大小，而元素的 width/height/padding/margin 属性用 em 的话则是相对于该元素的 font-size 大小。
    - rem：相对于根元素 html。
5. vw/vh布局  
    vw/vh是一个相对单位（类似 em 和 rem 相对单位），所以不同视口（屏幕）下，宽高可以一起变化完成适配。  
...
- **1px 边框问题**：由于设备像素比（dpr）不同，1px 在手机上显示较粗，可通过伪元素 + `transform: scale(0.5)` 或 `viewport` 缩放解决。
- **点击穿透/延迟**：使用 `touch` 事件或 `fastclick` 库消除点击延迟。
- **安全区域适配**：针对刘海屏、底部黑条等，使用 `env(safe-area-inset-top)`、`constant(safe-area-inset-top)` 处理。
...
<!-- .c1 .c4{} 与 .c3 .c4{} 哪个权重更重 -->
<!-- .c1 .c4{} 与 .c3 .c4{} 的权重相同。后写的生效，所以 .c3 .c4{} 会覆盖 .c1 .c4{}，.c3 .c4{} 生效 -->
```

> - `.c1 .c2` 是后代选择器，选择 `.c1` 里面的所有 `.c2` 元素。`p.c4` 不是 `.c2`，所以 `.c1 .c2{}` 不会命中 `p.c4`。但是 `p.c4` 会继承属性，所以 `.c1 .c2` 的样式可能会影响到 `p.c4`。
> - `.c2 .c3` 是后代选择器，选择 `.c2` 里面的所有 `.c3` 元素。

如果优先级（选择器权重）相同，则后出现的规则覆盖先出现的规则（“后来居上”原则），即选择晚声明的选择器。
...
- 缺点明显
  - **内容变形**：如果单纯使用百分比，当屏幕过宽时，文本行会变得非常长，导致阅读困难；当屏幕过窄时，图片和文字可能会挤在一起，甚至溢出。
  - **难以精确控制**：对于具有固定尺寸的元素（如图片、带边框的盒子），单纯使用百分比容易导致子元素撑破父容器（除非配合 `box-sizing: border-box;` 或使用 `calc()` 函数）。
  - **视觉效果不稳定**：在大屏显示器上，元素会被拉得很宽，可能会显得稀疏且缺乏设计感。

## 7. 行内布局的特点

行内布局是相邻的元素会在同一行上，宽度为元素的内容宽度
...
- inline（行内元素）:  
  ...
  - 多个行内元素会在一行内水平排列，只有当父容器的宽度不够时，才会自动换行到下一行。
  - 无法直接通过 CSS 设置 `width` 和 `height`。它们的宽度完全由内部内容的宽度（文字、图片）决定。
  - 可以部分设置 `margin,padding` 属性：可以设置水平方向上的（left,right），不可以设置垂直方向上的（top,bottom）
  - 常见的行内元素有：span,a,strong,em,input,img,br等
  - 只有设置行高才可以增加行内框的高度（line-height）

可以通过 `vertical-align` 属性来调整它们的对齐方式，常见值包括：

- middle（垂直居中对齐）
- top（顶部对齐）
- bottom（底部对齐）
- text-top（与父元素文字的顶部对齐）

```html
<div style="display: inline; line-height: 60px;">
    <span style="vertical-align: middle;">文本1</span>
    <span style="vertical-align: top;">文本2</span>
    <span style="vertical-align: bottom;">文本3</span>
</div>
```
...
1. 导航栏/菜单栏，菜单项等分分布。给父容器设置 `display: flex`，然后给每个菜单项设置 `flex: 1`。这样无论菜单项的文字内容多长，它们都能等分父容器的宽度。

**2、垂直居中**

2.1、使用场景：

1. 在卡片列表或者表单布局中，如果需要让左边的图标和右边的文字在垂直方向上居中对齐，我只需要给父容器设置 `display: flex` 和 `align-items: center`。
2. 在输入框旁边添加按钮会用到，用于调整水平高度
...
- **场景：** 主要用于**边框（border）**、**阴影（box-shadow）**、**1px 分割线**、**固定宽度的图标**或者**小范围不需要伸缩的布局**。
- **理由：** 这些元素通常需要绝对精确且不变形。如果用 rem，当屏幕变大时，整个边框等比放大，视觉上会显得很粗，不符合设计细节。

**2. 为什么用 rem？**

- **场景：** 主要用于**移动端适配**（H5 活动页、公众号文章）。
- **理由：** 为了实现**等比例缩放**。
  - *做法：* 通过 JS 监听屏幕宽度，动态设置 `document.documentElement.style.fontSize`（例如设置 1rem = 屏幕宽度的 1/10）。
  - *效果：* 设计稿上的 100px，在不同手机上渲染出来的物理像素不同，但视觉效果（占屏幕的比例）是一样的。开发者只需要将设计稿的 px 除以基准值换算成 rem 即可。
  - *优点：* 一套代码，所有手机等比例缩放，完美还原设计稿。

**3. 为什么用 em？**

- **场景：** 主要用于特定的**组件内部**，利用其“相对于父级”的特性。
...
## 16. position 为 absolute 的 div，假如它的父 div 的 position 不是 absolute、relative，那么这个 div 以谁为基准
...
[三栏布局](../css/usage/实现两栏、三栏.md#实现三栏)
...
## 29. 你怎么理解BFC？用过它解决过什么布局问题？

BFC（块级格式化上下文）是一个独立的布局环境，可以理解为一个容器，在这个容器中按照一定规则进行物品摆放，并且不会影响其它环境中的物品。如果一个元素符合触发BFC的条件，则BFC中的元素布局不受外部影响。

解决的问题：清除浮动、避免 margin 穿透、实现自适应两栏布局（左边固定右边自适应）
