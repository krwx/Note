...
- [加载图片](#加载图片)
  ...
  - [懒加载图片](#懒加载图片)
    - [使用 vue-lazyload](#使用-vue-lazyload)
      - [视口检查规则](#视口检查规则)
      - [参数说明](#参数说明)
    - [使用 `loading="lazy"` 属性](#使用-loadinglazy-属性)
    - [使用 `Intersection Observer API`](#使用-intersection-observer-api)
      - [使用 VueUse 的 `useIntersectionObserver`](#使用-vueuse-的-useintersectionobserver)
      - [使用原生的 `Intersection Observer API`](#使用原生的-intersection-observer-api)
...
传统方式（降级方案）：
1. 监听 window 的 scroll/resize 事件
2. 使用节流函数优化性能
3. 计算每个元素位置与视口关系
4. 检查元素是否进入：元素.top < 视窗高度 + 预加载距离
```
...
现代浏览器支持在 `<img>` 标签上使用 `loading="lazy"` 属性实现原生懒加载

**工作原理**：

1. **初始加载**：当页面首次加载时，浏览器会解析 HTML。对于设置了 `loading="lazy"` 的图片，浏览器只会获取极小的占位信息（通常只有几KB），而不会立即下载完整的图片文件。
2. **滚动触发**：浏览器会监测用户的滚动行为。当一个懒加载的图片**滚动到接近视口一定距离时**（这个距离由浏览器决定，通常是一个“视口高度”的缓冲区域），浏览器才会开始下载这张图片。
3. **加载完成**：图片下载完成后，会立刻显示出来，替换掉之前的占位符。

**例子**：

```vue
<template>
  <img src="@/assets/large-image.jpg" alt="Lazy Loaded Image" loading="lazy">
</template>
```

**注意事项**：

`loading="lazy"` 依赖于浏览器视口，而不是可滚动容器。如果 `.container` 设置了 `height: 100vh` 和 `overflow: auto`，那么每个`<img>`都会被视为“在视图中”，浏览器会立即获取所有这些图片。
...
<template>
    <div class="container">
        <!-- 使用 data-src 存储 image 的 src -->
        <img ref="imgRefs" v-for="imageSrc in imageList" :key="imageSrc" :data-src="imageSrc" class="imageItem"
            alt="logo"></img>
    </div>
</template>
...
const imgRefs = useTemplateRef('imgRefs');
