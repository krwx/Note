...
Vue Router 支持开箱即用的 _动态导入_（`import()`），这意味着你可以用动态导入代替静态导入：
...
> 注意
> **不要**在路由中直接使用[异步组件](https://cn.vuejs.org/guide/components/async.html)。  
> 即 `{path: '/login', component: someAsyncComponent}`  
> 异步组件仍然可以在路由组件里面使用。
