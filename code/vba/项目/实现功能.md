...
通过 `ActiveWindow.VisibleRange` 获取当前窗口的可见单元格区域，并显示其地址
...
Sub GetVisibleRange()
    ...
    If Not visibleRange Is Nothing Then
        MsgBox "当前窗口的可见单元格区域是: " & visibleRange.Address
    Else
        MsgBox "无法获取可见单元格区域", vbExclamation
    End If
End Sub
```
...
Sub GetSecondVisibleRow()
    ...
    If Not visibleRange Is Nothing Then
        If visibleRange.Rows.Count >= 2 Then
            MsgBox "当前窗口的可见单元格区域的第二行行号是: " & visibleRange.Rows(2).Row
        Else
            MsgBox "可见单元格区域没有第二行", vbExclamation
        End If
    Else
        MsgBox "无法获取可见单元格区域", vbExclamation
    End If
