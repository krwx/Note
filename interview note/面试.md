...
面试官好，我叫陈广鸿，今年26岁，毕业于武汉理工大学，从事软件开发已经三年多。上一份工作就职于汉睿科技有限公司，从事前端开发，主要负责无人零食贩售机后台管理系统的商品管理和订单管理的页面开发。再上一份工作就职于中兴通讯股份有限公司，注意负责UME云化智能管理系统的网络切片、以太网模块的页面开发，也负责一部分以太网模块的后端开发。我会使用Java进行后端开发，会使用react进行前端开发，能简单使用 python 进行开发。

### new

前端工程师：

面试官好，我叫陈广鸿，今年27岁，毕业于武汉理工大学，有5年开发经验。第一份工作就职于中兴通讯股份有限公司，负责 UME 云化智能管理系统的网页开发，根据客户需求实现业务相关功能，搭起了业务界面的通用框架供其他业务模块使用，基于公司自研拓扑组件实现业务的拓扑展示。目前就职于德迅货运代理公司，主要负责 SearatesView 系统的网页开发，实现了富文本邮件编辑与发送的功能，封装通用错误提示框，优化 vite 配置，搭起了 python 后端 Celery 的主程序。在技术方面，我能熟练掌握 Vue 全家桶进行项目开发，熟悉 Vite 构建工具的基本配置与优化，也熟悉 Python、Celery 开发。以上就是我的基本情况，谢谢。、

---

数据分析师：

面试官好，我叫陈广鸿，今年27岁，毕业于武汉理工大学，有5年开发经验。目前就职于德迅货运代理公司，主要负责 SeatratesView 系统的数据支持与后台任务框架搭建，我搭建了后端 Celery 任务框架实现查询、格式化、分析数据这几个任务的异步执行，并使用 pandas 对业务数据进行处理及分析，也负责前端界面的实现，实现了富文本邮件编辑与发送的功能。在技术方面，我能熟悉进行 Python 开发，能熟练使用 pandas 进行数据处理。以上就是我的基本情况，谢谢。
...
### 前端访问 flower 界面添加权限控制

原来是直接通过服务器地址访问 flower，后面需要加上权限控制，然后使用 window.open() 是直接打开页面的，传递不了参数给 django 后端。最后实现的方案是在 django 新建一个获取 token 的接口和一个访问 flower 的接口，前端先访问获取 token 的接口，django 对用户进行权限认证，然后生成一个 token，将 token 返回给前端，前端再用 token 访问 flower 的接口，这个接口对 token 进行解析，确认有权限之后再请求当前服务器的 flower 地址，将 flower 的响应返回给前端，前端渲染页面

### task 运行速度慢，添加数据库 index 后变快

### lighthouse 优化。找到有一个组件莫名其妙导入 axios
